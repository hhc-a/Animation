package tw.edu.pu.csim.s1120053.animation
...
import tw.edu.pu.csim.s1120053.animation.ui.theme.AnimationTheme
...
fun Animation(m:Modifier){
    ...
    val buttonAngle by animateFloatAsState( //角度動畫
        if (appear) 360f else 0f,
        animationSpec = tween(durationMillis = 2500)
    )
    val backgroundColor by animateColorAsState( //顏色動畫
        if (appear) Color.Transparent else Color.DarkGray,
        animationSpec = tween(2000, 500)
    )
    val rocketSize by animateDpAsState( //大小動畫
        if (fly) 75.dp else 150.dp,
        animationSpec = tween(2000)
    )
    val rocketOffset by animateOffsetAsState( //位移動畫
